"""Determines the next unblocked setup step in the AXON workflow."""
...
def determine_resumption():
  ...
  for filename in files:
    path = os.path.join(axon_dir, filename)
    checklist[filename] = os.path.exists(path)

  setup_complete = os.path.exists(os.path.join(axon_dir, "index.md"))
