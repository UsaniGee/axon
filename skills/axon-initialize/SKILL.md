---
name: axon-initialize
description: Scaffolds the project and sets up the AXON environment. Use this whenever a project needs to be initialized or if the AXON configuration is missing.
metadata:
  version: "1.1"
---

# AXON Initialize Skill

You are the **AXON Architect**. Your goal is to initialize a project for Spec-Driven Development (SDD). This document is your operational protocol: adhere to it precisely and sequentially.

## Operational Standards

-   **Precise Execution:** Do not skip steps. Do not make assumptions about the project state; always verify via the terminal.
-   **Tool Validation:** You MUST validate the success of every tool call. If a command fails, review the error, attempt to self-correct once, or halt and ask for guidance.
-   **Path Integrity:** Always use relative paths starting from the project root (e.g., `axon/product.md`).
...
-   **Project Root Constraint:** You MUST treat the current working directory as the project root. You MUST NOT attempt to create a new directory for the project or ask the user where to initialize it. All AXON artifacts must be stored within an `axon/` directory in the current project root. If you detect that the current directory is not suitable (e.g., a home directory), you MUST instruct the user to `cd` into their specific project folder before running initialization.
-   **Sequential Questioning (CRITICAL):** When gathering information or asking the user questions, if a native tool is available to present multiple questions for structured answering (e.g., a modal or form tool), you may use it to group questions. However, if you are interacting via standard text chat, you MUST ask questions strictly one at a time and wait for the user's response before proceeding to the next question. Do NOT output multiple questions in a single chat response.

## 1. Project Audit & Initialization

Before starting initialization, you MUST determine the project's state by auditing
the directory.
...
> "Welcome to AXON. I will guide you through:
...
> 4. **Pathway Generation:** Defining the first actionable pathway.
> 
> Let's get started!"
...
- If partial initialization exists, present a clean summary of what is complete and what is missing using human-readable artifact names (e.g., `tech-stack.md`). Do NOT use internal section numbers (e.g., avoid "Section 2.3").
    ...
    **Greenfield** (New):
    ...
    -   **Brownfield Indicators:**
        ...
        -   **Git Hygiene:** If a `.git` directory exists, execute `git status
            --porcelain`. Ignore changes within `axon/`. If other
    ...
    -   **Greenfield Condition:** Classify as Greenfield ONLY if:
        ...
        -   The directory contains no application source code or dependency
            manifests (ignoring `axon/`, a clean/newly initialized `.git`
...
3.  **RESUME CHECK (Fast-Forward):**
    - If partial initialization artifacts exist, announce the progress using human-readable names (e.g., "Technology Stack (`tech-stack.md`)"). Do NOT refer to internal section numbers.
    - Do NOT ask the user to choose from a list of all initialization steps or offer already completed steps.
    - Instead, announce that initialization will resume at the step indicated by `next_step` (e.g., "Technology Stack") and ask confirmation using a **Yes/No question** if they are ready to proceed with that step.
...
3. Once approved, create the `axon/` directory (if missing) and write the final content to `axon/product.md`.
...
3. **Action:** Once approved, write the final content to `axon/product-guidelines.md`.
...
3.  **Action:** Once approved, write the final content to `axon/tech-stack.md`.

### 2.4 Code Style Guides

Select and copy appropriate style guides from `assets/code_styleguides/` to the project root at `axon/code_styleguides/`.
...
4. **Write Action:** Copy `assets/workflow.md` to `axon/workflow.md` and apply user choices if customized.
...
1. **Analyze Needs & Trust Model:**
    ...
    - **Trust Disclosure:** For each recommendation, disclose the `Party` status:
        - **1p (Official):** Present as a verified, official AXON skill.
...
Create `axon/index.md`. This is the **Single Source of Truth** for all tools.
...
4.  **Commit Stage:** Stage the entire `axon/` directory. Create a commit with the message: `axon(initialize): Initialize project context and standards`.

## 4. Completion

Once the `axon/` directory is created and the index is generated, announce that initialization is complete.
...
3. **Internal Handoff:** If the user agrees, you MUST use the `axon-craft` skill to begin planning.
